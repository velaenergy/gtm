...
- **V10 — AI-grounded full drafting.** After available enrichment, the OpenAI writer receives bounded work context and rewrites one complete subject/body draft. The selected template supplies intent and reusable sender facts but is not send-ready copy. `workNote` remains the exact prospect-specific opener used in the generated body, and provider context is never permission to invent facts.
...
- **V22 — Active compose context.** Selecting a verified recipient promotes that address and its work/personal/provider metadata into the visible email field, saved draft, delivery target, and writer request. An explicit **Rewrite with AI** action always rewrites one complete grounded draft from the current profile, recipient, template, opener, subject, and body; greeting, founder story, ask, calendar URL, and sign-off each appear at most once.
...
- **V24 — LinkedIn launcher.** Supported LinkedIn profile and people-search pages expose one compact Vela launcher. Its user click opens the existing tab-scoped Chrome side panel through the background worker; unsupported LinkedIn routes show no launcher.
- **V25 — AI-complete delivery gate.** Every new profile starts one full-email AI writing attempt while the loading view remains visible. Workspace delivery, manual Gmail handoff, and scheduling remain disabled until that profile has a successful AI-generated subject and complete body; a missing key, failed request, stale profile result, or in-flight rewrite must fail closed. Template output may seed the writer but is never send-ready by itself.
...
| T25 | x | synchronize recipient context, make explicit AI rewriting full-draft, add LinkedIn launcher, and back up campaign workspace | V22,V23,V24,I4,I9 |
| T26 | x | move automatic full-email writing into profile loading and gate every delivery path on AI completion | V25,I4,I6,I10 |
