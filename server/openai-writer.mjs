...
Treat template.subjectPattern, template.bodyBlueprint, and currentDraft as a writing guide and source of approved sender facts, not immutable copy. Preserve the guide's purpose, the sender's identity, the exact calendar URL, and factual Vela claims, but rewrite the complete email for this prospect. Vary the subject, transitions, wording, paragraph count, and placement of the personalized detail. Do not reuse the same paragraph skeleton simply because the guide supplies one.

Write the body as plain text with 3-7 short, natural paragraph blocks chosen for this message. Separate paragraphs with one blank line. Do not hard-wrap lines inside a paragraph. Include a greeting, a grounded reason for reaching out, enough Vela context to make the ask credible, one clear 20-30 minute ask, the exact calendar URL when supplied, and a sign-off using the configured sender name. Aim for 90-170 words, but prefer natural writing over filling a fixed format. Use a concise, specific subject; avoid a generic fixed subject such as "Quick intro" when the profile supports something sharper.
...
const GENERATION_MODE_INSTRUCTIONS = `Regardless of generationMode, return a newly written complete subject, complete body, and grounded workNote in the required fields.`;
