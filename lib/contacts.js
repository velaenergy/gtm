...
export function buildContacts({ prospects = [], deliveryLog = [] } = {}) {
  ...
  return [...contacts.values()].map((contact) => {
    ...
    const operators = [...new Set(contact.activities
      .map((record) => record.operatorName || record.senderName || record.senderEmail || "")
      .filter(Boolean))];
    const normalized = {
      ...contact,
      lastContactAt,
...
export function filterContacts(contacts = [], { query = "", status = "all" } = {}) {
  ...
  return (Array.isArray(contacts) ? contacts : []).filter((contact) => {
    ...
    return [contact.name, contact.email, contact.company, contact.status, contact.source, contact.deliveryHealth, contact.bounceReason, ...(contact.operators || [])]
      .join(" ")
      .toLowerCase()
