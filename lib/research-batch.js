...
export function buildProspectAuditRequest(prospect = {}, gmailContext = "", audienceContext = {}) {
  const providerProfile = prospect.profile && typeof prospect.profile === "object" ? prospect.profile : {};
  const headline = providerProfile.headline || prospect.headline || "";
  const company = providerProfile.company || (prospect.company ? { name: prospect.company } : null);
  const experiences = Array.isArray(providerProfile.experiences) && providerProfile.experiences.length
    ? providerProfile.experiences
    : headline || prospect.company ? [{ title: headline, company: prospect.company || company?.name || "" }] : [];
  const profile = {
    ...providerProfile,
    name: providerProfile.name || prospect.name,
    headline,
    location: providerProfile.location || prospect.location,
    company,
    experiences,
  };
