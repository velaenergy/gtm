  ...
  for (const record of groups.flat()) {
    ...
    for (const recipient of recipients) {
      ...
      if (existingIndex !== undefined) {
        ...
        const existingHasOperator = Boolean(existing.operatorId || existing.operatorEmail || existing.operatorName);
        const incomingHasOperator = Boolean(record.operatorId || record.operatorEmail || record.operatorName);
        const preferIncomingOperator = incomingHasOperator && (!existingHasOperator || record.source === "supabase");
        if (preferIncomingOperator) {
...
export function mailboxHealthRows({ accounts = [], messages = [], syncStates = [], fromDate = "" } = {}) {
  const cutoff = validDate(fromDate)?.getTime() ?? Number.NEGATIVE_INFINITY;
  const mailboxes = new Map();
  const emailIndex = new Map();
  const ensureMailbox = ({ id = "", email = "" } = {}) => {
    const normalizedId = String(id || "").trim();
    const normalizedEmail = String(email || "").trim().toLowerCase();
    const existingKey = (normalizedId && mailboxes.has(normalizedId) ? normalizedId : "")
      || emailIndex.get(normalizedEmail)
      || normalizedId
      || normalizedEmail;
    if (!existingKey || !normalizedEmail.includes("@")) return null;
    if (!mailboxes.has(existingKey)) {
      mailboxes.set(existingKey, {
        id: normalizedId || normalizedEmail,
    ...
    const mailbox = mailboxes.get(existingKey);
    if (normalizedId && mailbox.id === mailbox.email) mailbox.id = normalizedId;
    emailIndex.set(normalizedEmail, existingKey);
    return mailbox;
  };

  for (const account of Array.isArray(accounts) ? accounts : []) ensureMailbox(account);
  for (const state of Array.isArray(syncStates) ? syncStates : []) {
    const mailbox = ensureMailbox({ id: state.gmail_account_id, email: state.gmail_accounts?.email || state.accountEmail });
    if (!mailbox) continue;
    mailbox.syncStatus = state.sync_status || "idle";
    mailbox.syncScope = state.sync_scope || "gtm_only";
    mailbox.lastSyncAt = [state.last_incremental_sync_at, state.last_full_sync_at, state.updated_at].filter(Boolean).sort().at(-1) || "";
    mailbox.lastError = String(state.last_error || "");
  }

  for (const message of Array.isArray(messages) ? messages : []) {
    const mailbox = ensureMailbox({
      id: message.gmailAccountId,
      email: message.accountEmail || (message.direction === "outgoing" ? message.senderEmail : ""),
    });
    if (!mailbox) continue;
    const occurredAt = String(message.occurredAt || "");
    const occurredTime = Date.parse(occurredAt);
    if (!Number.isFinite(occurredTime) || occurredTime < cutoff) continue;
    if (occurredAt > mailbox.lastActivityAt) mailbox.lastActivityAt = occurredAt;
    const threadId = String(message.gmailThreadId || message.gmailMessageId || "");
    if (message.direction === "outgoing" && ["initial", "follow_up"].includes(message.messageKind)) {
      mailbox.sentMessages += 1;
      if (threadId) mailbox.sentThreads.add(threadId);
    } else if (message.messageKind === "reply") {
      if (threadId) mailbox.repliedThreads.add(threadId);
    } else if (message.messageKind === "bounce") {
      mailbox.bounceSignals += 1;
      if (message.bounceReason === "policy_blocked") mailbox.policyBlocks += 1;
      else if (message.bounceType === "hard") mailbox.hardBounces += 1;
      else if (message.bounceType === "soft") mailbox.softBounces += 1;
    }
  }

  return [...mailboxes.values()].map((mailbox) => ({
    ...mailbox,
    sentThreads: mailbox.sentThreads.size,
    repliedThreads: mailbox.repliedThreads.size,
    replyRate: mailbox.sentThreads.size ? Math.round((mailbox.repliedThreads.size / mailbox.sentThreads.size) * 1000) / 10 : 0,
    coverage: mailbox.syncStatus === "syncing"
      ? "syncing"
      : mailbox.syncStatus === "error"
        ? "error"
        : mailbox.syncStatus === "complete" && mailbox.syncScope === "all_sent_threads"
          ? "complete"
          : "needs_sync",
  })).sort((a, b) => a.email.localeCompare(b.email));
}

export function summarizeMailboxHealth(rows = []) {
  const summary = (Array.isArray(rows) ? rows : []).reduce((totals, mailbox) => ({
    connectedMailboxes: totals.connectedMailboxes + 1,
    currentMailboxes: totals.currentMailboxes + (mailbox.coverage === "complete" ? 1 : 0),
    sentMessages: totals.sentMessages + (Number(mailbox.sentMessages) || 0),
    sentThreads: totals.sentThreads + (Number(mailbox.sentThreads) || 0),
    repliedThreads: totals.repliedThreads + (Number(mailbox.repliedThreads) || 0),
    bounceSignals: totals.bounceSignals + (Number(mailbox.bounceSignals) || 0),
    policyBlocks: totals.policyBlocks + (Number(mailbox.policyBlocks) || 0),
    hardBounces: totals.hardBounces + (Number(mailbox.hardBounces) || 0),
    softBounces: totals.softBounces + (Number(mailbox.softBounces) || 0),
    syncIssues: totals.syncIssues + (["error", "needs_sync"].includes(mailbox.coverage) ? 1 : 0),
  }), {
  ...
  return {
    ...summary,
    replyRate: summary.sentThreads ? Math.round((summary.repliedThreads / summary.sentThreads) * 1000) / 10 : 0,
  };
}
