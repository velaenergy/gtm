...
export function googleOAuthErrorMessage(error, { redirectUri = "" } = {}) {
  ...
  if (/user interaction required|interaction[_ ]required|login[_ ]required|consent[_ ]required/i.test(message)) {
    return "The selected Gmail session needs to be reconnected in Settings.";
  }
...
async function googleIdTokenNonce(cryptoImpl = globalThis.crypto) {
  if (!cryptoImpl?.getRandomValues || !cryptoImpl?.subtle?.digest) throw new Error("Secure Google nonce hashing is unavailable.");
  ...
  const hashedNonce = [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
  return { nonce, hashedNonce };
}

export function buildGoogleWebAuthUrl({ clientId = "", redirectUri = "", scopes = [], state = "", nonce = "", interactive = true, loginHint = "", includeIdToken = false } = {}) {
  ...
  url.searchParams.set("response_type", includeIdToken ? "id_token token" : "token");
  url.searchParams.set("scope", requestedScopes.join(" "));
  url.searchParams.set("include_granted_scopes", "true");
  url.searchParams.set("hd", "velaenergy.ai");
  url.searchParams.set("state", state);
  if (includeIdToken) {
    if (!nonce) throw new Error("Google authorization requires a hashed ID-token nonce.");
...
export function parseGoogleWebAuthResponse(responseUrl = "", { expectedState = "", requiredScopes = [], requireIdToken = false } = {}) {
  ...
  if (requireIdToken && !idToken) throw new Error("Google authorization did not return a Vela sign-in identity token.");
  ...
  return { token, idToken, expiresIn: Number(params.get("expires_in")) || 0, scopes: returnedScopes };
}
...
async function runGoogleWebAuth({ identity, clientId = "", scopes = [], interactive = false, loginHint = "", includeIdToken = false, fetchImpl = globalThis.fetch, cryptoImpl = globalThis.crypto } = {}) {
  ...
  const requestedScopes = normalizedScopes([...scopes, GOOGLE_USERINFO_EMAIL_SCOPE, ...(includeIdToken ? ["openid", "email", "profile"] : [])]);
  const state = randomOAuthState(cryptoImpl);
  const noncePair = includeIdToken ? await googleIdTokenNonce(cryptoImpl) : { nonce: "", hashedNonce: "" };
  const url = buildGoogleWebAuthUrl({ clientId, redirectUri, scopes: requestedScopes, state, nonce: noncePair.hashedNonce, interactive, loginHint, includeIdToken });
  let responseUrl;
  try {
  ...
  const authorization = parseGoogleWebAuthResponse(responseUrl, { expectedState: state, requiredScopes: requestedScopes, requireIdToken: includeIdToken });
  const account = await googleAccountForToken(authorization.token, fetchImpl);
  return { ...authorization, nonce: noncePair.nonce, account };
}

export async function chooseGoogleAccount(options = {}) {
  const result = await authorizeGoogleAccount(options);
  return result.account;
}

export async function authorizeGoogleAccount(options = {}) {
  return runGoogleWebAuth({ ...options, interactive: true, loginHint: "" });
}

export async function getGoogleWebAuthToken({ expectedEmail = "", interactive = false, ...options } = {}) {
  const result = await runGoogleWebAuth({ ...options, interactive: Boolean(interactive), loginHint: expectedEmail });
