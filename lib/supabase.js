...
export async function deactivateGmailAccount(account = {}, options = {}) {
  const id = String(account.id || "").trim();
  const email = String(account.email || "").trim().toLowerCase();
  if (!id || !/^\S+@\S+\.\S+$/.test(email)) throw new Error("Choose a valid Gmail account to remove.");
  const { data } = await authenticated("/rest/v1/rpc/deactivate_gmail_sender", {
    ...options,
    method: "POST",
    body: { p_account_id: id, p_account_email: email },
  });
  return data || { id, email, isActive: false };
}
...
export function gtmMessageRecordFromRow(row = {}) {
  return {
    ...
    operatorId: row.team_profiles?.id || row.captured_by || "",
    operatorEmail: row.team_profiles?.email || "",
    operatorName: row.team_profiles?.full_name || row.team_profiles?.email || "",
...
export async function sharedGtmEmailMessages({ accountId = "", includeBody = false, ...options } = {}) {
  ...
  const columns = [
    ...
    "team_profiles!gtm_email_messages_captured_by_fkey(id,email,full_name,avatar_url)",
  ].join(",");
...
function prospectIdentityAliases(prospect = {}) {
  const linkedIn = String(prospect.url || prospect.linkedinUrl || "").trim().toLowerCase().replace(/[?#].*$/, "").replace(/\/$/, "");
  const email = String(prospect.email || "").trim().toLowerCase();
  return [...new Set([linkedIn ? `linkedin:${linkedIn}` : "", email ? `email:${email}` : ""].filter(Boolean))];
}

function prospectIdentityFilter(identityKeys = []) {
  const value = `in.(${identityKeys.map((key) => `"${key.replaceAll('"', '\\"')}"`).join(",")})`;
  return `/rest/v1/prospects?select=identity_key&identity_key=${encodeURIComponent(value)}`;
}
...
export async function deleteSharedProspects(prospects = [], options = {}) {
  const session = await activeSupabaseSession(options);
  if (!session) throw new Error("Sign in with your Vela Energy account before clearing approvals.");
  const identityKeys = [...new Set((Array.isArray(prospects) ? prospects : []).flatMap(prospectIdentityAliases))];
  ...
  for (let offset = 0; offset < identityKeys.length; offset += 100) {
    ...
    const { data } = await authenticated(prospectIdentityFilter(batch), {
      ...options,
      method: "DELETE",
      headers: { Prefer: "return=representation" },
    });
    if (Array.isArray(data)) deleted.push(...data.map((row) => row.identity_key).filter(Boolean));
  }
  const remaining = [];
  for (let offset = 0; offset < identityKeys.length; offset += 100) {
    const batch = identityKeys.slice(offset, offset + 100);
    const { data } = await authenticated(prospectIdentityFilter(batch), options);
    if (Array.isArray(data)) remaining.push(...data.map((row) => row.identity_key).filter(Boolean));
  }
  if (remaining.length) throw new Error(`Supabase still contains ${remaining.length} matching approval record${remaining.length === 1 ? "" : "s"}. Reload and try again.`);
  return deleted;
}
