...
export async function deleteSharedProspects(prospects = [], options = {}) {
  ...
  const identityKeys = [...new Set((Array.isArray(prospects) ? prospects : []).map(prospectIdentity).filter(Boolean))];
  ...
  for (let offset = 0; offset < identityKeys.length; offset += 100) {
    ...
    const filter = `in.(${batch.map((key) => `"${key.replaceAll('"', '\\"')}"`).join(",")})`;
    const { data } = await authenticated(`/rest/v1/prospects?select=identity_key&identity_key=${encodeURIComponent(filter)}`, {
      ...options,
      method: "DELETE",
  ...
  const missing = identityKeys.filter((key) => !deletedKeys.has(key));
  if (missing.length) throw new Error(`Supabase did not delete ${missing.length} of ${identityKeys.length} approval${identityKeys.length === 1 ? "" : "s"}. Reload and try again.`);
  return deleted;
}
