...
import { calibrateTargetFit } from "./target-fit.js";
...
export function createProspect(input = {}, now = new Date().toISOString()) {
  ...
  const fitProfile = {
    ...(profile || {}),
    headline: profile?.headline || input.headline || "",
    experiences: Array.isArray(profile?.experiences) && profile.experiences.length
      ? profile.experiences
      : input.headline || input.company ? [{ title: input.headline || "", company: input.company || "" }] : [],
  };
  return {
    id,
    ...
    targetFit: input.targetFit && typeof input.targetFit === "object" ? calibrateTargetFit(input.targetFit, { profile: fitProfile }) : null,
