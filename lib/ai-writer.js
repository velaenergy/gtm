import { OUTREACH_SUBJECT, cleanText } from "./message.js";
...
export function mergeEnrichedProfile(profile = {}, enrichment = {}) {
  ...
  return {
    ...profile,
    name: cleanText(provider.name || profile.name),
    linkedinUrl: cleanText(provider.linkedinUrl || provider.linkedin_url || profile.linkedinUrl || profile.url),
...
export function fullDraftQualityIssues({ subject = "", body = "", workNote = "" } = {}, input = {}) {
  ...
  const issues = [];
  if (cleanSubject !== OUTREACH_SUBJECT) issues.push(`Use the standard subject: ${OUTREACH_SUBJECT}.`);
  ...
  if (/\b\d+\s*[–—]\s*\d+\b/.test(cleanBody)) issues.push("Use a regular hyphen in number ranges, such as 20-30.");
  if (/\bgrab any time here\b/i.test(cleanBody)) issues.push("Introduce the calendar naturally instead of saying grab any time here.");
  if (/\b(?:i saw your background|i(?:['’]m| am) reaching out because|especially the move from|your move from|that (?:mix|combination|blend) of|exactly the (?:kind|sort) of experience|teams like yours|from your side of the table)\b/i.test(cleanBody)) issues.push("Avoid resume-style synthesis and state one concrete current-work fact before the question.");
  if (!/\bi came across your profile and\b/i.test(cleanBody)) issues.push('Open with the human setup "I came across your profile and...".');
  if (/\b(?:you (?:are|lead|serve as|work as)|your (?:current )?(?:role|position|title))\b/i.test(cleanBody)) issues.push("Do not recite the prospect's job title or position.");
