...
export function researchBatchPagination(run = {}) {
  ...
  const sourcePulled = Math.max(pulled, Number(run.sourcePulledCount || run.source_pulled_count) || 0);
  return {
    page,
    perPage,
    total,
    pulled,
    nextPage: page + 1,
    hasNext: sourcePulled > 0 && page * perPage < total,
  };
}
...
export function researchFunnel(run = {}) {
  return [
    ...
    { key: "previously_contacted", label: "Already sent", value: Math.max(0, Number(run.previouslyContactedCount || run.previously_contacted_count) || 0) },
...
export function pendingReviewDrafts(prospects = []) {
  return (Array.isArray(prospects) ? prospects : []).filter((prospect) => prospect?.status === "ready");
}

export function reviewDrawerDrafts(prospects = [], activeId = "") {
  const items = (Array.isArray(prospects) ? prospects : [])
    .filter((prospect) => ["ready", "drafted"].includes(prospect?.status));
  const active = items.find((prospect) => prospect?.id === activeId);
  if (active?.status === "drafted") return items.filter((prospect) => prospect.status === "drafted");
  return items.filter((prospect) => prospect.status === "ready");
}

export function approvalSendSummary(sent = 0, failures = []) {
  const problems = (Array.isArray(failures) ? failures : []).filter(Boolean);
  if (!problems.length) return `${sent} message${sent === 1 ? "" : "s"} sent through the template sender${sent === 1 ? "" : "s"}.`;
  if (problems.length === 1) return `${sent} sent · ${problems[0]}`;
  return `${sent} sent · ${problems.length} need attention · First: ${problems[0]}`;
}

export function resolveDuplicateSendDecision(people = [], matches = [], decision = "cancel") {
  const recipients = new Set((Array.isArray(matches) ? matches : [])
    .map((match) => String(match?.recipient || "").trim().toLowerCase())
    .filter(Boolean));
  const approved = Array.isArray(people) ? people : [];
  if (decision === "override") return { proceed: true, override: true, people: approved, skippedCount: 0 };
  if (decision !== "skip") return { proceed: false, override: false, people: [], skippedCount: 0 };
  const sendable = approved.filter((person) => !recipients.has(String(person?.email || "").trim().toLowerCase()));
  return { proceed: true, override: false, people: sendable, skippedCount: approved.length - sendable.length };
}

export function nextReviewProspectId(prospects = [], currentId = "") {
  const items = Array.isArray(prospects) ? prospects : [];
  const currentIndex = items.findIndex((prospect) => prospect?.id === currentId);
  const remaining = items.filter((prospect) => prospect?.id !== currentId);
  if (!remaining.length) return "";
  if (currentIndex < 0) return String(remaining[0]?.id || "");
  return String(remaining[Math.min(currentIndex, remaining.length - 1)]?.id || "");
}
