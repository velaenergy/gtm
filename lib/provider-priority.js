...
export function enrichmentProvidersForAttempt(settings = {}, { contactOutDefault = true, contactOutOnly = false, hasInitialApolloResult = false } = {}) {
  const providers = configuredEnrichmentProviders(settings);
  if (contactOutOnly) return providers.filter((provider) => provider !== PROVIDER.APOLLO);
  if (!contactOutDefault) return hasInitialApolloResult ? [] : providers.filter((provider) => provider === PROVIDER.APOLLO);
  return hasInitialApolloResult ? providers.filter((provider) => provider !== PROVIDER.APOLLO) : providers;
}

export function configuredManualEnrichmentProviders(settings = {}) {
  return [
    settings.contactOutSessionEnabled ? PROVIDER.CONTACTOUT_SESSION : "",
    settings.contactOutApiKey ? PROVIDER.CONTACTOUT : "",
    settings.apolloApiKey ? PROVIDER.APOLLO : "",
