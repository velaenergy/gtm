...
export function normalizeContactOutSessionReveal(payload = {}, pending = {}) {
  ...
  const workStatuses = Object.fromEntries(workEmails.map((candidate) => [clean(candidate.value).toLowerCase(), verifiedInternalEmail(candidate, pending.verificationStatus) ? "verified" : "unverified"]));
  const personalStatuses = Object.fromEntries(personalEmails.map((candidate) => [clean(candidate.value).toLowerCase(), verifiedInternalEmail(candidate, pending.verificationStatus) ? "verified" : "unverified"]));
...
export async function previewContactOutSession(profile, { chromeApi = globalThis.chrome, includePhone = false } = {}) {
  ...
  const records = response.data?.profiles && typeof response.data.profiles === "object" ? Object.values(response.data.profiles) : [];
  const record = records[0] || {};
  const emailCandidates = Array.isArray(record.emails) ? record.emails : [];
  const phoneCandidates = Array.isArray(record.phones) ? record.phones : [];
  ...
  const resolvedDescriptor = {
    ...
    companies: (Array.isArray(record.companies) ? record.companies : descriptor.companies).map((value) => finiteNumber(value)).filter((value) => value > 0).slice(0, 8),
  };
  await appendDiagnostic({
