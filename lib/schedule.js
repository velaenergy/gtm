...
export const SCHEDULED_SEND_KIND = Object.freeze({
  INITIAL: "initial",
  FOLLOW_UP: "follow-up",
});
...
export function scheduledSendKind(job = {}) {
  return job?.kind === SCHEDULED_SEND_KIND.FOLLOW_UP ? SCHEDULED_SEND_KIND.FOLLOW_UP : SCHEDULED_SEND_KIND.INITIAL;
}

export function scheduledSendMatches(job = {}, query = "", prospect = {}) {
  const needle = String(query || "").trim().toLowerCase();
  if (!needle) return true;
  const kind = scheduledSendKind(job);
  const haystack = [
    prospect?.name,
    prospect?.headline,
    job?.recipients?.join(" "),
    ...
    job?.templateId,
    kind,
    kind === SCHEDULED_SEND_KIND.FOLLOW_UP ? `follow up automatic sequence step ${job?.sequenceStep || ""}` : "initial one time scheduled",
...
export function createScheduledSend(input = {}, now = new Date()) {
  ...
  return {
    ...
    kind: input.kind === SCHEDULED_SEND_KIND.FOLLOW_UP ? SCHEDULED_SEND_KIND.FOLLOW_UP : SCHEDULED_SEND_KIND.INITIAL,
    ...
    followUps: Array.isArray(input.followUps) ? input.followUps.slice(0, 3).map((step) => ({ templateId: String(step?.templateId || ""), body: String(step?.body || "") })).filter((step) => step.body) : [],
    followUpCadenceDays: Math.min(30, Math.max(1, Number(input.followUpCadenceDays) || 3)),
