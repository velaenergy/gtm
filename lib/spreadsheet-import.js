...
export function mappedRowsToProspects({ rows = [], mapping = [], settings = {}, source = "Spreadsheet import", xlsx = globalThis.XLSX } = {}) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const template = outreachTemplate(settings);
    const draft = applyTemplate(template, templateVariables(profile, settings, workNote, template));
    prospects.push({
      url,
      name: profile.name,
      ...
      workNote,
      subject: emailSentAt ? String(values.subject || "").trim() || draft.subject : OUTREACH_SUBJECT,
      ...
      importedAt: now,
      emailSentAt,
      activity: [{ type: "imported", detail: source, at: now }, ...(emailSentAt ? [{ type: "sent", detail: "Imported historical send", at: emailSentAt }] : [])],
    });
  });
  return { prospects, rejected };
}

export function historicalDeliveryRecords(prospects = []) {
  return (Array.isArray(prospects) ? prospects : [])
    .filter((prospect) => isEmail(prospect.email) && prospect.emailSentAt)
    .map((prospect, index) => ({
      id: `import-${String(prospect.email).toLowerCase()}-${prospect.emailSentAt}-${index}`,
      status: "sent",
      mode: "imported",
      recipients: [String(prospect.email).toLowerCase()],
      subject: prospect.subject || "",
      completedAt: prospect.emailSentAt,
      updatedAt: prospect.emailSentAt,
      prospectId: prospect.id || prospect.url || prospect.email,
    }));
}
