...
export async function peopleSearchViaApollo(filters = {}, options = {}) {
  ...
  const listMapping = { job_title: "person_titles", seniority: "person_seniorities", location: "person_locations", industry: "organization_industries" };
  for (const [key, target] of Object.entries(listMapping)) { const value = filters[key]; if (Array.isArray(value) ? value.length : clean(value)) body[target] = value; }
  const scalarMapping = { company: "q_organization_name", keyword: "q_keywords" };
  for (const [key, target] of Object.entries(scalarMapping)) {
    const value = clean(Array.isArray(filters[key]) ? filters[key].join(" ") : filters[key]);
...
export function broadenApolloPeopleFilters(filters = {}) {
  const broadened = { ...filters, include_similar_titles: true };
