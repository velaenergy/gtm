...
export async function enrichViaApollo(profile, options = {}) {
  ...
  if (profile?.providerId) details.id = clean(profile.providerId);
  const payload = await apolloRequest("https://api.apollo.io/v1/people/bulk_enrich", { ...options, method: "POST", query: { reveal_personal_emails: true, reveal_phone_number: Boolean(options.includePhone) }, body: { details: [details] } });
