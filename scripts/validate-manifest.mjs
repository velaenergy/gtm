...
assert.ok(Number(manifest.minimum_chrome_version) >= 114, "Chrome 114+ is required for the Side Panel API.");
...
assert.deepEqual(manifest.oauth2.scopes, ["https://www.googleapis.com/auth/gmail.send"]);
assert.match(manifest.oauth2.client_id, /^[A-Za-z0-9_-]+\.apps\.googleusercontent\.com$/, "Google OAuth client ID must use Google's Chrome extension client format.");
assert.equal(manifest.oauth2.client_id, "185496922277-aoi4hsbke7el382uk559uotlvo9478fj.apps.googleusercontent.com", "The published Gmail OAuth client must remain wired to the release manifest.");
